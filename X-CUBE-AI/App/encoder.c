...
#define AI_NET_OBJ_INSTANCE g_encoder
 
#undef AI_ENCODER_MODEL_SIGNATURE
#define AI_ENCODER_MODEL_SIGNATURE     "0x267d2af1d26e3e4a31ae191a16d32221"
...
#undef AI_ENCODER_N_BATCHES
#define AI_ENCODER_N_BATCHES         (1)

static ai_ptr g_encoder_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_encoder_weights_map[1] = AI_C_ARRAY_INIT;
...
/* Array#13 */
AI_ARRAY_OBJ_DECLARE(
  latent_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 2048, AI_STATIC)

/* Array#14 */
AI_ARRAY_OBJ_DECLARE(
  latent_Transpose_0_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
  NULL, NULL, 2048, AI_STATIC)
...
/* Array#35 */
AI_ARRAY_OBJ_DECLARE(
  latent_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
...
AI_TENSOR_OBJ_DECLARE(
  ...
  24, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 4, 1), AI_STRIDE_INIT(4, 4, 4, 4, 16),
...
AI_TENSOR_OBJ_DECLARE(
  ...
  25, 0x0,
  AI_SHAPE_INIT(4, 1, 4, 1, 32), AI_STRIDE_INIT(4, 4, 4, 128, 512),
...
/* Tensor #27 */
AI_TENSOR_OBJ_DECLARE(
  latent_Transpose_0_output, AI_STATIC,
  27, 0x0,
  AI_SHAPE_INIT(4, 1, 64, 1, 32), AI_STRIDE_INIT(4, 4, 4, 256, 256),
  1, &latent_Transpose_0_output_array, NULL)
...
AI_TENSOR_CHAIN_OBJ_DECLARE(
  latent_Transpose_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &latent_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &latent_Transpose_0_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  latent_Transpose_0_layer, 1,
  TRANSPOSE_TYPE, 0x0, NULL,
  transpose, forward_transpose,
  &latent_Transpose_0_chain,
  NULL, &latent_Transpose_0_layer, AI_STATIC, 
...
AI_TENSOR_CHAIN_OBJ_DECLARE(
  ...
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &latent_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &latent_weights, &latent_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &latent_scratch0, NULL)
)
...
AI_TENSOR_CHAIN_OBJ_DECLARE(
  ...
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &relu_2_output, &conv2d_5_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_1_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)
...
AI_TENSOR_CHAIN_OBJ_DECLARE(
  ...
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &relu_output, &conv2d_2_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 149248, 1, 1),
    149248, NULL, NULL),
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 98560, 1, 1),
    98560, NULL, NULL),
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool encoder_configure_activations(
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool encoder_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_weights_map(g_encoder_weights_map, 1, params)) {
    /* Updating weights (byte) offsets */
    
    conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv2d_weights_array.data = AI_PTR(g_encoder_weights_map[0] + 0);
    conv2d_weights_array.data_start = AI_PTR(g_encoder_weights_map[0] + 0);
    conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv2d_bias_array.data = AI_PTR(g_encoder_weights_map[0] + 512);
    conv2d_bias_array.data_start = AI_PTR(g_encoder_weights_map[0] + 512);
    conv2d_1_weights_array.format |= AI_FMT_FLAG_CONST;
    conv2d_1_weights_array.data = AI_PTR(g_encoder_weights_map[0] + 640);
    conv2d_1_weights_array.data_start = AI_PTR(g_encoder_weights_map[0] + 640);
    conv2d_1_bias_array.format |= AI_FMT_FLAG_CONST;
    conv2d_1_bias_array.data = AI_PTR(g_encoder_weights_map[0] + 12928);
    conv2d_1_bias_array.data_start = AI_PTR(g_encoder_weights_map[0] + 12928);
    conv2d_2_weights_array.format |= AI_FMT_FLAG_CONST;
    conv2d_2_weights_array.data = AI_PTR(g_encoder_weights_map[0] + 13056);
    conv2d_2_weights_array.data_start = AI_PTR(g_encoder_weights_map[0] + 13056);
    conv2d_2_bias_array.format |= AI_FMT_FLAG_CONST;
    conv2d_2_bias_array.data = AI_PTR(g_encoder_weights_map[0] + 17152);
    conv2d_2_bias_array.data_start = AI_PTR(g_encoder_weights_map[0] + 17152);
    conv2d_3_weights_array.format |= AI_FMT_FLAG_CONST;
    conv2d_3_weights_array.data = AI_PTR(g_encoder_weights_map[0] + 17280);
    conv2d_3_weights_array.data_start = AI_PTR(g_encoder_weights_map[0] + 17280);
    conv2d_3_bias_array.format |= AI_FMT_FLAG_CONST;
    conv2d_3_bias_array.data = AI_PTR(g_encoder_weights_map[0] + 50048);
    conv2d_3_bias_array.data_start = AI_PTR(g_encoder_weights_map[0] + 50048);
    conv2d_4_weights_array.format |= AI_FMT_FLAG_CONST;
    conv2d_4_weights_array.data = AI_PTR(g_encoder_weights_map[0] + 50304);
    conv2d_4_weights_array.data_start = AI_PTR(g_encoder_weights_map[0] + 50304);
    conv2d_4_bias_array.format |= AI_FMT_FLAG_CONST;
    conv2d_4_bias_array.data = AI_PTR(g_encoder_weights_map[0] + 99456);
    conv2d_4_bias_array.data_start = AI_PTR(g_encoder_weights_map[0] + 99456);
    conv2d_5_weights_array.format |= AI_FMT_FLAG_CONST;
    conv2d_5_weights_array.data = AI_PTR(g_encoder_weights_map[0] + 99712);
    conv2d_5_weights_array.data_start = AI_PTR(g_encoder_weights_map[0] + 99712);
    conv2d_5_bias_array.format |= AI_FMT_FLAG_CONST;
    conv2d_5_bias_array.data = AI_PTR(g_encoder_weights_map[0] + 116096);
    conv2d_5_bias_array.data_start = AI_PTR(g_encoder_weights_map[0] + 116096);
    latent_weights_array.format |= AI_FMT_FLAG_CONST;
    latent_weights_array.data = AI_PTR(g_encoder_weights_map[0] + 116352);
    latent_weights_array.data_start = AI_PTR(g_encoder_weights_map[0] + 116352);
    latent_bias_array.format |= AI_FMT_FLAG_CONST;
    latent_bias_array.data = AI_PTR(g_encoder_weights_map[0] + 149120);
    latent_bias_array.data_start = AI_PTR(g_encoder_weights_map[0] + 149120);
    return true;
  }
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_ENCODER_MODEL_NAME,
      .model_signature   = AI_ENCODER_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_encoder_get_error(ai_handle network)
{
...
AI_API_ENTRY
ai_error ai_encoder_create_and_init(
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ai_error err;
  ai_network_params params;

  err = ai_encoder_create(network, AI_ENCODER_DATA_CONFIG);
  if (err.type != AI_ERROR_NONE) {
    return err;
  }
  
  if (ai_encoder_data_params_get(&params) != true) {
    err = ai_encoder_get_error(*network);
    return err;
  }
#if defined(AI_ENCODER_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_ENCODER_DATA_WEIGHTS_COUNT)
  /* set the addresses of the weight buffers */
  for (ai_u16 idx=0; weights && idx<params.map_weights.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
  }
#endif
  if (ai_encoder_init(*network, &params) != true) {
    err = ai_encoder_get_error(*network);
  }
  return err;
}
...
AI_API_ENTRY
ai_buffer* ai_encoder_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_encoder_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    AI_NETWORK_OBJ(network)->magic = AI_MAGIC_CONTEXT_TOKEN;
...
AI_API_ENTRY
ai_handle ai_encoder_destroy(ai_handle network)
{
  ...
  ai_handle network, const ai_network_params* params)
{
  ...
  ok &= encoder_configure_weights(net_ctx, params);
  ok &= encoder_configure_activations(net_ctx, params);
...
AI_API_ENTRY
ai_i32 ai_encoder_forward(ai_handle network, const ai_buffer* input)
{
